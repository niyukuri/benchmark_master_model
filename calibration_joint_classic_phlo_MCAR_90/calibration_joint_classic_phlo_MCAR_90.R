...
work.dir <-  "/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90"  # on CHPC
...
df <- read.csv("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/Results.benchmark.epi.mm.stats_280_123.csv")
...
epi.metrics <- benchmark_data %>%
  dplyr::select(contains("metr.")) # 39

med.epi.metrics <-  colMedians(as.matrix(epi.metrics), na.rm = TRUE)

classic.features <- benchmark_data[,40:66] # 27

med.classic.features <- colMedians(as.matrix(classic.features), na.rm = TRUE)


# 
# 
MCAR.phylo.features.cov.90 <- benchmark_data %>% # 36
  dplyr::select(contains("MCAR.90"))


med.MCAR.phylo.features.cov.90 <- colMedians(as.matrix(MCAR.phylo.features.cov.90), na.rm = TRUE)


med.classic.MCAR.phylo.features <- c(med.classic.features, med.MCAR.phylo.features.cov.90)
...
simpact4ABC.classic.MCAR.phylo.90 <- function(inputvector = inputvector){
  ...
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/needed.functions.RSimpactHelp.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/compute.summary.statistics.classic.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/compute.summary.statistics.phylo.MCAR.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/advanced.transmission.network.builder.R")
  ...
  if (length(results) == 0){
    ...
    if (as.numeric(results["eventsexecuted"]) >= (as.numeric(cfg.list["population.maxevents"]) - 1)) {
      ...
      MCAR.phylo.stats <- tryCatch(compute.summary.statistics.phylo.MCAR(simpact.trans.net = simpact.trans.net,
      ...
      outputvector <- c(epi.behav.stats, MCAR.phylo.stats)
...
# d <- simpact4ABC.classic.MCAR.phylo.90(inputvector = inputvector)

source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MCAR_90/calibration.ABC.R")


calib.class.MCAR.phylo.90.results <- calibration.ABC(model.sim = simpact4ABC.classic.MCAR.phylo.90,
                                                     sum_stat_obs = med.classic.MCAR.phylo.features,
                                                     simpact_prior = simpact_prior, 
                                                     design.points = 1120,
                                                     seed.val = 6975,
                                                     n_cores = 56)

save(calib.class.MCAR.phylo.90.results, file = "calib.class.MCAR.phylo.90.results.RData")
