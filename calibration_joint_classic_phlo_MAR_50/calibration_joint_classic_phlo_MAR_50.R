...
work.dir <-  "/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50"  # on CHPC
...
df <- read.csv("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/Results.benchmark.epi.mm.stats_280_123.csv")
...
# 
MAR.phylo.features.cov.50 <- benchmark_data %>% # 36
  dplyr::select(contains("MAR.a.50"))


med.MAR.phylo.features.cov.50 <- colMedians(as.matrix(MAR.phylo.features.cov.50), na.rm = TRUE)


med.classic.MAR.phylo.features <- c(med.classic.features, med.MAR.phylo.features.cov.50)
...
simpact4ABC.classic.MAR.phylo.50 <- function(inputvector = inputvector){
  ...
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/needed.functions.RSimpactHelp.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/compute.summary.statistics.classic.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/compute.summary.statistics.phylo.MAR.R")
  source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/advanced.transmission.network.builder.R")
  ...
  if (length(results) == 0){
    ...
    if (as.numeric(results["eventsexecuted"]) >= (as.numeric(cfg.list["population.maxevents"]) - 1)) {
      ...
      outputvector <- c(epi.behav.stats, mar.phylo.stats)
...
# d <- simpact4ABC.classic.MAR.phylo.50(inputvector = inputvector)

source("/home/dniyukuri/lustre/calibration_joint_classic_phlo_MAR_50/calibration.ABC.R")


calib.class.MAR.phylo.50.results <- calibration.ABC(model.sim = simpact4ABC.classic.MAR.phylo.50,
                                                    sum_stat_obs = med.classic.MAR.phylo.features,
                                                    simpact_prior = simpact_prior, 
                                                    design.points = 1120,
                                                    seed.val = 6975,
                                                    n_cores = 56)

save(calib.class.MAR.phylo.50.results, file = "calib.class.MAR.phylo.50.results.RData")
