...
advanced.transmission.network.builder <- function(datalist = datalist, endpoint = 40){
  ...
  for(i in 1:length(seeds.id)){
    ...
    for (j in 1:length(ids)) {
      ...
      location.x <- c(location.x, location.x.val)
      location.y <- c(location.y, location.y.val)
      vl <- c(vl, vl.val) # place holder >> median of VL
